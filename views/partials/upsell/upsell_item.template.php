<?php 
    ...
    $price 		= woo_j_get_price( $product['woo_product'], $wcproduct->get_regular_price() );	
	$qty 		= $product['quantity'] ?? 1;
	//Check if we have a discount or the upsell has full price
	if( $product['discount'] )
	{
		$percentage = $product['discount_type'] == 1 ? $product['discount'] : ( $product['discount'] * 100 ) / get_post_meta( $product['id'] , '_regular_price', true ) ; 
		$price 		= round( $price - ( ( $price / 100 ) * $percentage ) , 2);
		
	}else{

		$price 		= woo_j_get_price( $product['woo_product'], $wcproduct->get_price() );	
	}
            ...
            <div class='wc-timeline-product-data flex-row-start'>	
						...
						<div class='image'>	
							...
							<?php elseif( $product['woo_product']->is_on_sale() ): ?>	
											<div class="discounted">
												<?php echo wjc__( esc_html( woo_j_conf('label_on_sale') ), 'label_on_sale' ) ?>
											</div>						
							<?php endif; ?>	
						...
						<div class='heading flex-column-start'>
								...
								<div style="position:relative;width:100%;">
										<div class='upsell-heading'><?php echo wjc__( $product['heading'], 'upsell_heading_' . $product['id'] ) ?></div>
                                        ...
                                        <div class="upsell-text">
                                            <?php echo wjc__( $product['text'], 'upsell_text_' . $product['id'] ) ?>
                                        </div>											
								</div>
                        ...
                        <div class="wc-timeline-product-add">
                                ...
                                <a <?php echo do_action( 'wjufw_add_to_cart_parameters', $product['woo_product'] ); ?> title="Add to cart" href="<?php echo $product_action ?>" data-upsell-type="<?php echo $upsell_type ?>" data-upsell="true" data-quantity="<?php echo $qty ?>" class="button product_type_simple <?php echo $class	?>" data-product_id="<?php echo esc_attr( $product['id'] ) ?>" 
											...
											<?php if( $is_add_to_cart ): ?> 
												<?php echo wjc__( esc_html( woo_j_conf('text_add_to_cart') ), 'text_add_to_cart' ) ?>
											<?php else: ?>
												<?php echo wjc__( esc_html( woo_j_conf('text_go_to_product') ?? 'view' ), 'text_go_to_product' ) ?>
											<?php endif; ?>
