(function($) {	
            ...
            $('.wc-upsellator-product-search').each( function() {
                ...
                $this.select2({
                        ...
                        allowClear: true
                                ...
                                $( document ).trigger( "wooj:product:changed", [ $this.val(), $this.closest('.upsell-gift-wrapper--container') ] );                        
                });
             });
    ...
    */
    $( document ).on( "wooj:product:changed", function( e, product_id, container ) {        
               
                if( !product_id ) return                    
                
                const already_exist = $('.upsell-gift-wrapper--container').not( container ).find('.selected_product').filter( ( index, element ) => $(element).attr('data-id') == product_id );
                
                if( already_exist.length ){
                        
                        const product_search_select = $(container).find('.product-search')

                        product_search_select.val( product_search_select.attr('data-id') ).trigger('change');
                        ...
                        if( selected_item )
                        {      
                                $(container).find('.wc-timeline-product-name').text( selected_item.base_name );
                                $(container).find('.image img').attr('src', selected_item.img );
                                $(container).find('.image img').attr('srcset', '' );
                                $(container).find('.preview-price').text( selected_item.price );
                                $(container).find('.preview-base-price').val( selected_item.price );                           
